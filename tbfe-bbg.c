...
int tbfe_bbg_ciphertext_deserialize(tbfe_bbg_ciphertext_t* ciphertext, const uint8_t* src) {
  ...
  memcpy(ciphertext->eddsa.sig, src, crypto_sign_BYTES);
  src += crypto_sign_BYTES;

  memcpy(ciphertext->eddsa_pk.key, src, crypto_sign_PUBLICKEYBYTES);
  src += crypto_sign_PUBLICKEYBYTES;
...
void tbfe_bbg_ciphertext_serialize(uint8_t* serialized, tbfe_bbg_ciphertext_t* ciphertext) {
  ...
  memcpy(serialized, ciphertext->eddsa.sig, crypto_sign_BYTES);
  serialized += crypto_sign_BYTES;

  memcpy(serialized, ciphertext->eddsa_pk.key, crypto_sign_PUBLICKEYBYTES);
  serialized += crypto_sign_PUBLICKEYBYTES;
}
...
unsigned tbfe_bbg_ciphertext_size(const tbfe_bbg_ciphertext_t* ciphertext) {
  ...
  return sizeof(uint32_t) + sizeof(uint32_t) + (ciphertext_count * BBG_CIPHERTEXT_SIZE) +
         SECURITY_PARAMETER + crypto_sign_BYTES + crypto_sign_PUBLICKEYBYTES;
}
...
#pragma omp parallel reduction(| : result_status) num_threads((k+1)/2)
  {
    ...
    // Private vector for each thread to store the generated ciphertexts.
    vector_t ctx_private = {NULL, 0, 0};
    if (vector_init(&ctx_private, ceil(k / omp_get_num_threads()))) {
      ret = BFE_ERROR;
      goto clear_thread;
    }
    ...
    ret = bbg_init_identity_from(&identity_tau_i, tau_i_depth, &tau); // Get path to 'tau'
    if (ret) {
...
#pragma omp for schedule(static)
    for (size_t i = 0; i < k; ++i) {
      ...
      // ciphertexts
      ret = bbg_encapsulate(ct, _key.k, &public_key->pk, &ciphertext->eddsa_pk, &public_key->params,
                            &identity_tau_i);
      if (ret) {
        goto clear_ciphertext;
      }

      // Accumulate all ciphertexts generated by one thread
      if (vector_add(&ctx_private, ct)) {
        ret = BFE_ERROR;
        goto clear_ciphertext;
      }
      continue;
    ...
    // Copy the generated ciphertexts from the private vectors of the threads.
    // Executing the loop ordered ensures that the ciphertexts are added in increasing order of the
    // hash functions.
#pragma omp for ordered
    for (int i = 0; i < omp_get_num_threads(); ++i) {
#pragma omp ordered
      vector_copy(ciphertext->Cs, &ctx_private);
    }

  clear_identity_tau_i:
    bbg_clear_identity(&identity_tau_i);
  clear_thread:
    vector_clear(&ctx_private);
    result_status |= ret;
  }
...
bool tbfe_bbg_eddsa_sig_are_equal(tbfe_bbg_ciphertext_t* l, tbfe_bbg_ciphertext_t* r) {
  return memcmp(l->eddsa.sig, r->eddsa.sig, crypto_sign_BYTES) == 0;
}
#endif
